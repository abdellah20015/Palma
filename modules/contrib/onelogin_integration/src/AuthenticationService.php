...
class AuthenticationService implements AuthenticationServiceInterface {
   ...
   */
  public function __construct(ConfigFactoryInterface $config_factory, \OneLogin_Saml2_Auth $one_login_saml_2_auth, EntityTypeManagerInterface $entity_type_manager, UserServiceInterface $user) {
  ...
  public function processLoginRequest() {
    // Get the configuration data necessary.
    ...
    if (empty($this->oneLoginSaml2Auth->getNameId())) {
    ...
    $saml_attributes = $this->oneLoginSaml2Auth->getAttributes();

    if (!empty($saml_attributes)) {
      $usernameMapping = $config->get('username');
      $mailMapping     = $config->get('email');
    ...
    if (empty($email) && strpos($this->oneLoginSaml2Auth->getNameId(), '@')) {
      $email = $this->oneLoginSaml2Auth->getNameId();
    }

    if (empty($username) && $config->get('username_from_email')) {
      $username = str_replace('@', '.', $email);
    }

    // Get the configuration for the matcher.
    $matcher = $config->get('account_matcher');
  ...
  public function syncRoles($user, array $saml_attributes) {
    ...
    $role_mapping = $config->get('role');
    $role_delimiter = $config->get('onelogin_role_delimiter');
    $roles = [];
    ...
    foreach ($available_roles as $role_machine_name => $role_nice_name) {
      $drupal_role_mappings[$role_machine_name] = explode(',', $config->get('role_' . $role_machine_name));
    }
